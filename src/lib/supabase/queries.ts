...
type TeamSafe = Pick<Database['public']['Tables']['teams']['Row'], 'team_id' | 'team_name'>;
export type TeamWithMembers = { team: TeamSafe; members: MemberRow[] };
...
export async function registerTeam(payload: RegistrationInsert): Promise<TeamWithMembers> {
  // 1) Insert team with plaintext password
  const { data: team, error: teamError } = await supabase
    .from('teams')
    .insert({ team_name: payload.team_name.trim(), pass: payload.pass.trim() })
    .select('team_id, team_name') // do not select pass back out
    .single();
...
export async function getTeamByEmail(email: string): Promise<TeamWithMembers | null> {
  ...
  // 1) locate a member with that email
  ...
  // 2) fetch team with embedded members (no password)
