...
import { Search, Filter, Eye, CheckCircle, XCircle, Clock, Code, Github, Globe } from "lucide-react";
...
export default function AdminSubmissionsPage() {
  ...
  const filteredSubmissions = submissions.filter(submission => {
    const matchesSearch = submission.team.team_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         ...
                         (submission.member?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Total Submissions", value: submissions.length, color: "cyber-blue-400" },
          { label: "Pending", value: submissions.filter(s => s.status === "PENDING").length, color: "warning-orange" },
          { label: "Accepted", value: submissions.filter(s => s.status === "ACCEPTED").length, color: "matrix-green" },
          { label: "Rejected", value: submissions.filter(s => s.status === "REJECTED").length, color: "alert-red" },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, y: 20 }}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-cyber-blue-400/20">
                <th className="text-left py-4 px-6 text-sm font-semibold text-gray-400">Team</th>
                <th className="text-left py-4 px-6 text-sm font-semibold text-gray-400">Problem</th>
                <th className="text-left py-4 px-6 text-sm font-semibold text-gray-400">Member</th>
                <th className="text-center py-4 px-6 text-sm font-semibold text-gray-400">Type</th>
                <th className="text-center py-4 px-6 text-sm font-semibold text-gray-400">Status</th>
                <th className="text-center py-4 px-6 text-sm font-semibold text-gray-400">Score</th>
                ...
                filteredSubmissions.map((submission, index) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <div className="font-semibold text-gray-200">{submission.team.team_name}</div>
                      <div className="text-xs text-gray-400">
                        {new Date(submission.submitted_at).toLocaleString()}
                      </div>
                    </td>
                    <td className="py-4 px-6 text-gray-300">{submission.problem.title}</td>
                    <td className="py-4 px-6">
                      <div className="text-gray-300">{submission.member?.name || "N/A"}</div>
                      <div className="text-xs text-gray-400">{submission.member?.email || "N/A"}</div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold">
                        ...
                        <span className={
                          submission.status === "ACCEPTED" ? "text-matrix-green" :
                          submission.status === "REJECTED" ? "text-alert-red" :
                          "text-cyber-blue-400"
                        }>
                          {submission.status}
                        </span>
                    ...
                    <td className="py-4 px-6 text-center font-bold text-gray-200">
